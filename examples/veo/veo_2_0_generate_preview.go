...
func runVeo20GeneratePreview() {
	service := qiniu.NewService(strings.TrimSpace(os.Getenv("QINIU_API_KEY")))
	ctx := context.Background()

	op, err := service.Operation(xai.Model("veo-2.0-generate-preview"), xai.GenVideo)
	if err != nil {
	...
	op.Params().
		Set("Prompt", "A timelapse of stars moving across a clear desert night sky, tripod shot").
	...
	results, err := xai.Wait(ctx, service, resp, func(resp xai.OperationResponse) {
		if taskID := resp.TaskID(); taskID != "" {
			fmt.Printf("  [veo-2.0-generate-preview] polling task: %s\n", taskID)
		}
	})
	if err != nil {
		fmt.Println("Error:", err)
	...
	fmt.Printf("veo-2.0-generate-preview: videos=%d\n", results.Len())
