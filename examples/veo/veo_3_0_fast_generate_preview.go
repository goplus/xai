...
func runVeo30FastGeneratePreview() {
	service := qiniu.NewService(strings.TrimSpace(os.Getenv("QINIU_API_KEY")))
	ctx := context.Background()

	op, err := service.Operation(xai.Model("veo-3.0-fast-generate-preview"), xai.GenVideo)
	if err != nil {
	...
	op.Params().
		Set("Prompt", "A neon-lit cyberpunk street in the rain, camera dolly forward, dramatic atmosphere").
	...
	results, err := xai.Wait(ctx, service, resp, func(resp xai.OperationResponse) {
		if taskID := resp.TaskID(); taskID != "" {
			fmt.Printf("  [veo-3.0-fast-generate-preview] polling task: %s\n", taskID)
		}
	})
	if err != nil {
		fmt.Println("Error:", err)
	...
	fmt.Printf("veo-3.0-fast-generate-preview: videos=%d\n", results.Len())
