...
func runVeo30GeneratePreview() {
	...
	// Prompt is required; other params are optional (use gemini.* constants when needed)
	op.Params().
		Set("Prompt", "A golden retriever running on a beach at sunset, cinematic lighting, slow motion").
		...
		Set("Resolution", gemini.Res1080p).
	...
	results, err := xai.Wait(ctx, service, resp, func(resp xai.OperationResponse) {
		if taskID := resp.TaskID(); taskID != "" {
			fmt.Printf("  [veo-3.0-generate-preview] polling task: %s\n", taskID)
		}
	})
	if err != nil {
		fmt.Println("Error:", err)
	...
	fmt.Printf("veo-3.0-generate-preview: videos=%d\n", results.Len())
