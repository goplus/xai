...
func runVeo31GeneratePreview() {
	...
	op.Params().
		Set("Prompt", "A panoramic aerial shot of autumn forest and river, smooth cinematic motion").
	...
	results, err := xai.Wait(ctx, service, resp, func(resp xai.OperationResponse) {
		if taskID := resp.TaskID(); taskID != "" {
			fmt.Printf("  [veo-3.1-generate-preview] polling task: %s\n", taskID)
		}
	})
	if err != nil {
		fmt.Println("Error:", err)
	...
	fmt.Printf("veo-3.1-generate-preview: videos=%d\n", results.Len())
